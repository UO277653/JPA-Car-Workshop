...
public class InvoicingSteps {
	...
	private InvoicingService service = Factory.service
		...
		if (workordersIds.isEmpty()) {
			workordersIds = ((List<WorkOrderDto>) this.ctx
					.get(TestContext.Key.WORKORDERS)).stream()
							.filter(wo -> wo.status.equals("FINISHED"))
							.map(wo -> wo.id)
	...
	public void anInvoiceIsCreated() {
		...
		assertTrue( updated.stream()
						.allMatch(wo -> wo.status.equals("INVOICED"))
			);

		/* with proper invoice_id */
		assertTrue(updated.stream()
						.allMatch(wo -> wo.invoiceId.equals( invoice.id ))
			);
